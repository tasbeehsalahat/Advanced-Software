...
const joinjob = async (req, res) => {
    try {
        ...
        const sql = `SELECT flag FROM jobs WHERE job_title = "${job_title}" AND company_name = "${company_name}"`;

        connection.execute(sql, [job_title, company_name], (err, result) => {
            if (err) {
                return res.json(err);
            }
console.log(result);
            if (result.length === 0) {
                return res.json("Error occurred: Job not found");
            }

            if (result[0].flag === 1) {
                return res.json("This job has already been taken");
            } else {
                const s = `UPDATE jobs SET flag = flag + 1 WHERE job_title = ? AND company_name = ?`;

                connection.execute(s, [job_title, company_name], (error, result) => {
                    if (error) {
                        return res.json(error);
                    }

                    const sql2 = `UPDATE users SET job_title = ? WHERE email = ?`;

                    connection.execute(sql2, [job_title, email], (err, result) => {
                        if (err) {
                            if (err.errno == 1062) {
                                return res.status(409).json("You have already joined this job");
                            }
                            return res.json(err);
                        }
                        return res.json("You have joined the job. Congratulations!");
                    });
                });
            }
        });
...
module.exports = {statusTask,updateuser,join,shownotification,match,informations,LendCenter,LendMaterial,chooseMaterial,joinjob} ;
